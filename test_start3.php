...
if($fe)
{
		...
		$sql8 = "INSERT INTO cf_timer(studentid , testid , start , spent , spentold) VALUES(?, ?, NOW(), ?, ?)";
    	$q8 = $pdo->prepare($sql8);
    	$q8->execute(array($username , $testid , '0' , '0'));
}
			...
			$('.prev').hide();
								...
								$.ajax({
									...
									if(html==0){
										...
										$('#questions').fadeIn()
										$('#questions').text("YOUR TIME IS FINISHED AND YOUR TEST HAS BEEN SUBMITTED, YOU WILL BE AUTO LOGOUT NOW!")
										var delay = 9000;
